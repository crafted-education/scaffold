...
ColumnGroupCell.prototype.setColumnWidths = function(widths) {

	if(widths.length !== this._childColumns.length) {
		throw new Error('The number of widths supplied does not match the number of columns in the group.');
	}
...
ColumnGroupCell.prototype.removeChildColumn = function(column) {
	//Need to check to make sure the column group hasn't been deleted
	//here to help prevent some edge cases that can happen during deletion
	...
	if(this.isEmpty()) {
	...
	else if(this._childColumns.length === 1) {
      //If there is only a single column left,
	  ...
	  var remainingColumn = this._childColumns[0];
	  var childCells = remainingColumn.getChildCells().slice();

	  if(parentColumn && childCells && childCells.length > 0) {
		  for (var i = 0; i < childCells.length; i++) {
			  var childCell = childCells[i];
			  //The order we do this in matters, since deleting
			  //the last cell in the column can end up deleting the
			  //column and the column group it is in
			  parentColumn.addChildCell(childCell, this);
			  remainingColumn.removeChildCell(childCell);
		  }
		  this.delete();
	  }
	}
...
ColumnGroupCell.prototype.resizeColumnsForNewColumn = function(newColumn, beforeIndex) {
	...
	this.setColumnWidths(newColumnWidthCombination);
...
ColumnGroupCell.prototype.getNextSibling = function() {
...
ColumnGroupCell.prototype.getPreviousSibling = function() {
	if(!this._parentColumn) return null;
...
ColumnGroupCell.name = 'ColumnGroupCell'; //IE doesn't support implicit name on function yet
